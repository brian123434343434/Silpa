...
{
    public class Ruh_Tipo_SancionDALC
    {
        private Configuracion objConfiguracion;

        public Ruh_Tipo_SancionDALC()
        {
        ...
        public DataTable Listar_Ruh_Tipo_Sancion(string strDescripcion)
        {
            try
            {
                ...
                DbCommand cmd = db.GetStoredProcCommand("BAS_LISTA_RUH_TIPO_SANCION", parametros);
        ...
        public void Insertar_Ruh_Tipo_Sancion(string strDescripcion, decimal deEstado)
        {
            try
            {
                SqlDatabase db = new SqlDatabase(objConfiguracion.SilpaCnx.ToString());
                object[] parametros = new object[] { strDescripcion, deEstado };
                DbCommand cmd = db.GetStoredProcCommand("BAS_INSERTAR_RUH_TIPO_SANCION", parametros);
        ...
        public void Actualizar_Ruh_Tipo_Sancion(decimal iID, string strDescripcion, decimal deEstado)
        {
            try
            {
                SqlDatabase db = new SqlDatabase(objConfiguracion.SilpaCnx.ToString());
                object[] parametros = new object[] { iID, strDescripcion, deEstado };
                DbCommand cmd = db.GetStoredProcCommand("BAS_ACTUALIZAR_RUH_TIPO_SANCION", parametros);
                db.ExecuteNonQuery(cmd);
            }
            catch (SqlException sql)
            {
        ...
        /// </summary>
        /// <param name="iID">Codigo de la adquisicion</param>
        public void Eliminar_Ruh_Tipo_Sancion(decimal iID)
        {
            try
            {
                SqlDatabase db = new SqlDatabase(objConfiguracion.SilpaCnx.ToString());
                object[] parametros = new object[] { iID };
                DbCommand cmd = db.GetStoredProcCommand("BAS_ELIMINAR_RUH_TIPO_SANCION", parametros);
                db.ExecuteNonQuery(cmd);
            }
            catch (SqlException sql)
            {
