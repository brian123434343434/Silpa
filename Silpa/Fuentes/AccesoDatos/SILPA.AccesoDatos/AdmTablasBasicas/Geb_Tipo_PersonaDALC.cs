...
{
    public class Geb_Tipo_PersonaDALC
    {
        private Configuracion objConfiguracion;

        public Geb_Tipo_PersonaDALC()
        {
        ...
        public DataTable Listar_Geb_Tipo_Persona(string strDescripcion)
        {
            try
            {
                ...
                DbCommand cmd = db.GetStoredProcCommand("BAS_LISTA_GEB_TIPO_PERSONA", parametros);
        ...
        {   
            try
            {
                ...
                DbCommand cmd = db.GetStoredProcCommand("BAS_INSERTAR_GEB_TIPO_PERSONA", parametros);
        ...
        public void Actualizar_Geb_Tipo_Persona(decimal iID, string strDescripcion, decimal deEstado)
        {
            try
            {
                ...
                DbCommand cmd = db.GetStoredProcCommand("BAS_ACTUALIZAR_GEB_TIPO_PERSONA", parametros);
                db.ExecuteNonQuery(cmd);
            }
            catch (SqlException sql)
            {
        ...
        public void Eliminar_Geb_Tipo_Persona(decimal iID)
        {
            try
            {
                ...
                DbCommand cmd = db.GetStoredProcCommand("BAS_ELIMINAR_GEB_TIPO_PERSONA", parametros);
                db.ExecuteNonQuery(cmd);
            }
            catch (SqlException sql)
            {
