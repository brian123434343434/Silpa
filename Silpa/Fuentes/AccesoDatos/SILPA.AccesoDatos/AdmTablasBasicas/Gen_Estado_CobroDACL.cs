...
{
    public class Gen_Estado_CobroDACL
    {
        private Configuracion objConfiguracion;
        public Gen_Estado_CobroDACL()
        {
        ...
        public DataTable Listar_Gen_Estado_Cobro(string strDescripcion)
        {
            try
            {
                ...
                DbCommand cmd = db.GetStoredProcCommand("BAS_LISTA_GEN_ESTADO_COBRO", parametros);
        ...
        public void Insertar_Gen_Estado_Cobro(string strNombre, string strDescripcion, byte byEstado)
        {
            ...
            {
                SqlDatabase db = new SqlDatabase(objConfiguracion.SilpaCnx.ToString());
                object[] parametros = new object[] { strNombre, strDescripcion, byEstado };
                DbCommand cmd = db.GetStoredProcCommand("BAS_INSERTAR_GEN_ESTADO_COBRO", parametros);
        ...
        public void Actualizar_Gen_Estado_Cobro(int iID, string strNombre, string strDescripcion, byte byEstado)
        {
            ...
            {
                SqlDatabase db = new SqlDatabase(objConfiguracion.SilpaCnx.ToString());
                object[] parametros = new object[] { iID, strNombre, strDescripcion, byEstado };
                DbCommand cmd = db.GetStoredProcCommand("BAS_ACTUALIZAR_GEN_ESTADO_COBRO", parametros);
                db.ExecuteNonQuery(cmd);
            }
            catch (SqlException sql)
            {
        ...
        public void Eliminar_Gen_Estado_Cobro(int iID)
        {
            try
            {
                ...
                DbCommand cmd = db.GetStoredProcCommand("BAS_ELIMINAR_GEN_ESTADO_COBRO", parametros);
                db.ExecuteNonQuery(cmd);
            }
            catch (SqlException sql)
            {
